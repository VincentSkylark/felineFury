...
async function applyClosure(js: string, chunk: any): Promise<{ code: string }> {
  try {
    ...
    console.warn('Closure Compiler not available (Java missing?), using original code:', error);
    return { code: js }; // Fallback to original code
  }
}
  ...
  return {
    ...
    transformIndexHtml: {
      ...
      handler: async (html: string, ctx?: IndexHtmlTransformContext): Promise<string> => {
        ...
        const minifiedHtml = await htmlMinify(cssInHtml, options);
